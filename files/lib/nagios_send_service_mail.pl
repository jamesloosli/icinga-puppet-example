...
my $mail_sender        = "Icinga Monitoring <icinga\@instantchannelinc.com>";
...
# image we need to set a valid host name and service name below.
# ########################################################################
my $test_host          = "susie114";    # existing host in Nagios
my $test_service       = "os_cpu_load"; # existing services in Nagios
...
# must be valid so the script can pick up the graph image in Nagiosgraph.
# ########################################################################
...
sub create_message_html {
  ...
  if (defined($o_addurl)) {
    ...
    # If the graph image wasn't empty, we add an additional link to Nagiosgraph
    if ($o_format eq "graph" && $graph_type ne "gif") {
      $html_msg .=  ", see also <a href=\"$ngraph_cgiurl?host=" . urlencode($o_hostname) 
                . "&service=" . urlencode($o_servicedesc) . "&geom=". $graph_img_size ."\">Nagiosgraph</a>\n"; }
  }
...
sub create_graph_image {
  ...
  my $fhandle = File::Temp->new(UNLINK =>1) or verb("create_graph_image: Cannot create the temporary image file.");
  $fhandle->autoflush(1);
  ...
  verb("create_graph_image: Encoded Nagiosgraph image file, format: ".$graph_type."\n");
...
# ##################### End of nagios_send_service_mail.pl ####################
